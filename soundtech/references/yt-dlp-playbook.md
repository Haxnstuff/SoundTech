...
- **The fix is authentication, one-time**: either `--cookies-from-browser "firefox:C:\Users\<you>\AppData\Roaming\zen\Profiles\<prof>"` (Firefox-based browsers incl. Zen aren't known by name — pass the profile path; Chrome/Edge app-bound encryption often fails, Vivaldi/Firefox/Zen are reliable), or a cookies.txt export — see SKILL.md.
- **Authenticated downloads additionally need the EJS challenge solver**: yt-dlp ≥2026.08 + `pip install -U "yt-dlp[default]"` + a JS runtime. The soundtech script always passes `--js-runtimes node`. Symptom without it: `The page needs to be reloaded` / `n challenge solving failed`. Verified empirically: no player client bypasses the age gate without auth (default/android/ios/tv/tv_embedded/web_embedded all tested).
